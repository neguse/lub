...
class Shadow11 {
  ...
  public static function onFrame() {
    ...
    var dvsR = Io.loadText("samples/data/11_shadow_depth.vs.slang");
    var dfsR = Io.loadText("samples/data/11_shadow_depth.fs.slang");
    var svsR = Io.loadText("samples/data/11_shadow_scene.vs.slang");
    var sfsR = Io.loadText("samples/data/11_shadow_scene.fs.slang");
    var dvs: String = dvsR.text; var dvsv: Int = dvsR.version;
    var dfs: String = dfsR.text; var dfsv: Int = dfsR.version;
    var svs: String = svsR.text; var svsv: Int = svsR.version;
    var sfs: String = sfsR.text; var sfsv: Int = sfsR.version;
    ...
    var shadowMap = Gfx.useTexture("shadow_map", SHADOW_SIZE, SHADOW_SIZE,
        Gfx.RGBA8, null, 1, { target: true, filter: Gfx.NEAREST, wrap: Gfx.CLAMP });
    var shadowDepth = Gfx.useTexture("shadow_depth", SHADOW_SIZE, SHADOW_SIZE,
        Gfx.DEPTH16, null, 1, { target: true, filter: Gfx.NEAREST, wrap: Gfx.CLAMP });
    ...
    Gfx.draw(Std.int(casters.length / STRIDE),
             { verts: casterBuf, uniforms: { light_mvp: lmvp } },
             { shader: depthShader, depth: true, depth_write: true, cull: Gfx.NONE });
    Gfx.endPass();
    ...
    Gfx.draw(Std.int(scene.length / STRIDE),
             ...
             { shader: sceneShader, depth: true, depth_write: true, cull: Gfx.NONE });
    Gfx.endPass();
  }
}
