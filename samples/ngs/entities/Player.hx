...
class Player {
	...
	static inline var HOX = 5;
	static inline var HOY = 5;
	static inline var HW = 5;
	static inline var HH = 7;
	static inline var SPEED = 6;
	static inline var SLOW = 3;
	...
	public function update(world:World, input:InputSnapshot):Void {
		if (alive) {
			...
			if (x + W > Viewport.X + Viewport.W)
				x = Viewport.X + Viewport.W - W;
			if (y < Viewport.Y)
				y = Viewport.Y;
			if (y + H > Viewport.Y + Viewport.H)
				y = Viewport.Y + Viewport.H - H;
			// 射撃は trigger。押しっぱなし自動連射ではなく、手連射で弾を出す。
			if (input.menu)
				world.spawn(Faction.PlayerBullets, new Bullet(x, y));
			if (invincible > 0)
				invincible--;
		} else {
			dying++;
	...
	public function isFinished():Bool
		return !alive && lives <= 0 && dying > GAMEOVER_FRAMES;
