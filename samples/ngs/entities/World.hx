...
class World {
	...
	public function tick(input:InputSnapshot):Void {
		...
		while (ei < enemySlots.length) {
			var s = enemySlots[ei];
			if (s.entity.update(this, input))
				ei++
			else
				enemySlots.splice(ei, 1);
		}
	}
	...
	public function resolveCollisions():Void {
		for (s in enemySlots) {
			...
			for (b in playerBullets) {
				if (checked >= 16)
					break; // 原典は先頭 16 bullet slot のみ判定
				checked++;
				var bullet:Bullet = cast b;
				if (bullet.dead)
					continue;
				if (overlap(s.entity.bounds(), b.bounds())) {
					if (en.onDamage(this, 1))
						bullet.dead = true;
					break;
				}
			}
		...
		if (player.alive && player.invincible == 0) {
			for (s in enemySlots) {
				if (s.faction == Faction.Effects || slotDead(s))
					continue;
