...
class Play implements Scene {
  ...
  public function draw(dl: DrawList): Void {
    ...
    if (phase == 0) {
      ...
      if (timer > 0x3c) Game.font.drawString(dl, 0x13c - 200, 100, "g", white);
      if (timer > 0x78) Game.font.drawString(dl, 0x15a - 200, 100, "s", white);
      if (timer > 0xb4) Game.font.drawString(dl, 0x12e - 200, 300, "g.o.!", white);
      drawHud(dl, white);
      return;
    }
