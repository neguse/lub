...
class Breakout3d10 {
  ...
  static function circleHitsRect(cx: Float, cy: Float, r: Float, rect: Dynamic): Bool {
    return cx + r > rect.x0 && cx - r < rect.x1 &&
           cy + r > rect.y0 && cy - r < rect.y1;
  }
  ...
  static function updateGame() {
    ...
    if (resetDown && !resetWasDown) resetGame();
    ...
    if (ballStuck) {
      ...
      if (Input.keyDown("space") || launchTimer > 1.0) launchBall();
      return;
    }
    ...
    if (ballY + BALL_R < -1.0) {
      lives = lives - 1;
      if (lives <= 0) resetGame() else resetBall();
  ...
  static function addBox(out: Array<Float>, cx: Float, cy: Float, cz: Float,
                         sx: Float, sy: Float, sz: Float, base: Array<Float>) {
  ...
  static function buildVertices(): Array<Float> {
    ...
    addBox(out, paddleX, PADDLE_Y, -0.10,
           PADDLE_W, PADDLE_H, PADDLE_D, [0.94, 0.96, 0.86, 1.0]);
    addSphere(out, ballX, ballY, -0.20, BALL_R, [1.0, 0.95, 0.65, 1.0]);

    for (i in 1...lives + 1) {
      addSphere(out, -0.88 + (i - 1) * 0.08, -0.94, -0.15,
                0.025, [0.95, 0.32, 0.36, 1.0]);
    }
    var scoreShow = score < 12 ? score : 12;
    for (i in 1...scoreShow + 1) {
      addBox(out, 0.48 + (i - 1) * 0.04, -0.94, -0.12,
             0.022, 0.055, 0.04, [0.26, 0.82, 0.62, 1.0]);
    }
  ...
  static function makeMvp(t: Float): lua.Table<Int, Float> {
    ...
    var cx = Math.cos(pitch); var sx = Math.sin(pitch);
    var ry: Array<Float> = [
      cy, 0, sy, 0,
      0, 1, 0, 0,
      -sy, 0, cy, 0,
  ...
  public static function onFrame() {
    ...
    var vsR = Io.loadText("samples/data/10_breakout3d.vs.slang");
    var fsR = Io.loadText("samples/data/10_breakout3d.fs.slang");
    ...
    Gfx.draw(Std.int(verts.length / STRIDE),
             { verts: vbuf, uniforms: { mvp: makeMvp(cameraT) } },
