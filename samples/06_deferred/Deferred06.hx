...
class Deferred06 {
	...
	public static function onFrame() {
		var gvsR = Io.loadText("samples/06_deferred/data/06_gbuffer.vs.slang");
		var gfsR = Io.loadText("samples/06_deferred/data/06_gbuffer.fs.slang");
		var gvertsR = Io.loadFloats("samples/06_deferred/data/06_gbuffer.verts.lua");
		var vvsR = Io.loadText("samples/06_deferred/data/06_view.vs.slang");
		var vfsR = Io.loadText("samples/06_deferred/data/06_view.fs.slang");
		var vvertsR = Io.loadFloats("samples/06_deferred/data/06_view.verts.lua");
		...
		if (gvs == null || gfs == null || gverts == null || vvs == null || vfs == null || vverts == null)
			return;

		// G-buffer attachments. Two render-target textures of the same size.
		var gbuf0 = Gfx.useTexture("gbuf0", RT_W, RT_H, Gfx.RGBA8, null, 1, {filter: Gfx.LINEAR, wrap: Gfx.CLAMP, target: true});
		...
		Gfx.draw(3, {verts: bG}, {shader: shG, depth: false, cull: Gfx.NONE});
		Gfx.endPass();
