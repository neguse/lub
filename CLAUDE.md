...
- サンプルは実行例であってテスト対象ではない。サンプルに自己テストを書かず、samples 専用のテストも置かない。検査は `tests/lua/` の runtime テストと golden に限る(runtime に検証段が入るまでの方針)
...
- verify: `cd web && npm run dev`(localhost:5173)起動 → 別プロセスで `LUB_URL=http://localhost:5173/ npm run verify`(playwright + chromium swiftshader)。A1-A4=初期描画/hot reload、A5=全サンプル切替、A6=C# 増分編集が commit ACK まで貫通、A7=診断のエディタ内表示と生成 Lua タブ、A8=C# 補完/hover(レイテンシ観測ログ付き)、A9=`tests/lua` の記録順と TransientBuffer のテスト、lubx の InstanceBatch3d(instance の struct の並びが WGSL でも同じか)と SpriteBatch の 1 pass 内の複数 flush のテストを player で WebGPU backend に走らせる。
