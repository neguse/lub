...
- ドキュメントは平易な語で現状だけを書く。開発 jargon(spike 等)・完了注記(完遂・GO 確定 等)・強調記法(bold)を現状ドキュメントに書かない(bold は docs-lint が検査)。記録(`docs/log/` 等)の本文は当時のまま残す。
...
- verify: `cd web && npm run dev`(localhost:5173)起動 → 別プロセスで `LUB_URL=http://localhost:5173/ npm run verify`(playwright + chromium swiftshader)。A1-A4=初期描画/hot reload、A5=全サンプル切替、A6=C# 増分編集が commit ACK まで貫通、A7=診断のエディタ内表示(C#/Haxe)、A8=C# 補完/hover(レイテンシ観測ログ付き)。
