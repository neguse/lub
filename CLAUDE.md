...
- verify: `cd web && npm run dev`(localhost:5173)起動 → 別プロセスで `LUB_URL=http://localhost:5173/ npm run verify`(playwright + chromium swiftshader)。A1-A4=初期描画/hot reload、A5=全サンプル切替、A6=C# 増分編集が commit ACK まで貫通、A7=診断のエディタ内表示と生成 Lua タブ、A8=C# 補完/hover(レイテンシ観測ログ付き)、A9=`tests/lua` の記録順と TransientBuffer のテストを player で WebGPU backend に走らせる。
