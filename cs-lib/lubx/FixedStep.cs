...
public class FixedStep
{
    ...
    private static string[] SCAN_KEYS = new string[]
    {
    ...
    {
        for (int i = 0; i < SCAN_KEYS.Length; i++)
        {
            if (SCAN_KEYS[i] == key)
            {
                return i;
    ...
    private void latchEdges()
    {
        for (int i = 0; i < SCAN_KEYS.Length; i++)
        {
            if (Input.key_pressed(SCAN_KEYS[i]))
            {
                pendingKeyPressed[i] = true;
            }
            if (Input.key_released(SCAN_KEYS[i]))
            {
        ...
        for (int b = 1; b <= 3; b++)
        {
            if (Input.mouse_pressed(b))
            {
                pendingMousePressed[b] = true;
            }
            if (Input.mouse_released(b))
            {
