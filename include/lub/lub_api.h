...
typedef struct LubPassOpts {
  ...
  // と、buffer / texture は束縛の名前ごと)。どちらかが uniforms を渡していれ
  // ば、どちらにも無い member は 0 (どちらも渡さない draw は uniform を書き
  // 換えず、前の draw の値が残る)。値は BeginPass の時点で写すので、渡した
  // Dictionary を後で書き換えても pass には効かない。buffer / texture は draw
  // のたびに引くので、 pass の中で宣言し直した内容も見える。EndPass で消
  // え、Dispatch には効かない。
