...
console.log("[verify] launching", {
  URL,
  HEAD,
  exe: exe || "(playwright default)",
});
...
try {
  fs.copyFileSync(shot01, LEGACY_SCREENSHOT);
} catch {}
const c1 = classify(shot01);
console.log("[verify] A1 buckets", c1);
if (
  !check(
    "A1 initial render (orange triangle)",
...
const triangleCs = fs.readFileSync(
  path.resolve("..", "samples", "01_triangle", "Triangle01.cs"),
  "utf8",
);
const redClearCs = triangleCs.replace(
  "{ 0.1, 0.1, 0.2, 1.0 }",
  "{ 0.9, 0.05, 0.05, 1.0 }",
);

try {
  if (redClearCs === triangleCs)
    throw new Error("ClearColor literal not found in Triangle01.cs");
  ...
  const redAppeared = c3.redish / c3.total > 0.1;
  if (
    !check(
      "A3 C# edit (red clear)",
...
try {
  await selectTabAndReplace(
    "01_triangle/data/01_triangle.verts.lua",
    smallVerts,
  );
  await page.waitForTimeout(DEBOUNCE_WAIT_MS + 1500);
  ...
  if (
    !check(
      "A4 verts edit (smaller triangle)",
      greenShrank,
...
const samples = [
  ...
  { name: "01_triangle", minNonBlack: 0.1 },
  { name: "02_vertex_color", minNonBlack: 0.1 },
  { name: "03_texture", minNonBlack: 0.1 },
  { name: "04_mvp", minNonBlack: 0.05 },
  { name: "05_postprocess", minNonBlack: 0.2 },
  { name: "06_deferred", minNonBlack: 0.1 },
  { name: "07_compute", minNonBlack: 0.1 },
  { name: "08_gltf", minNonBlack: 0.1 },
  { name: "09_breakout", minNonBlack: 0.1 },
  { name: "10_breakout3d", minNonBlack: 0.1 },
  { name: "11_shadow", minNonBlack: 0.1 },
  ...
  await page.evaluate(
    (ms) =>
      new Promise((resolve, reject) => {
        const t = setTimeout(
          () => reject(new Error("timeout waiting for playerReady")),
          ms,
        );
        const h = (e) => {
  ...
  try {
    ...
    if (drewSomething) {
      if (known) {
        ...
        console.warn(
          `[A5/${name}] unexpected pass — remove from KNOWN_FAILING`,
        );
      }
    ...
    if (known) {
      console.warn(
        `[A5/${name}] expected failure: threw ${e.message}; not gating CI`,
      );
    } else {
...
try {
  ...
  const flappySrc = fs.readFileSync(
    path.resolve("..", "samples", "17_flappy", "Flappy17.cs"),
    "utf8",
  );
...
try {
  ...
  const flappyBroken = flappySrc.replace(
    "velocityY = 3.0;",
    "velocityY = thisIsUndefined;",
  );
  if (flappyBroken === flappySrc)
    throw new Error("A7 C# edit marker not found");
  ...
  check(
    "A7 generated Lua tab present",
    files.includes(".lub/17_flappy.lua"),
...
try {
  ...
  const r = await page.evaluate(() => {
    ...
    const specContent =
      src.slice(0, at + marker.length) +
      " Gfx." +
      src.slice(at + marker.length);
    const compPos = at + marker.length + " Gfx.".length;
    const t0 = performance.now();
    const comp = window.__lubTest.csQuery.complete(
      "Flappy17.cs",
      specContent,
      compPos,
    );
  ...
  console.log(
    `[verify] A8 complete ${Math.round(r.compMs)}ms, hover ${Math.round(r.hoverMs)}ms`,
  );
  if (
    !check(
      "A8 C# completion lists lub API member",
