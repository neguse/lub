...
// optional. Other attributes (TANGENT / COLOR_0 / JOINTS / WEIGHTS /
// TEXCOORD_1+) are ignored with a one-time warning.
...
int lub_load_gltf(lua_State *L) {
  ...
  if (prim->type != cgltf_primitive_type_triangles) {
    fprintf(stderr,
            "load_gltf: mesh[0].primitives[0] is not triangles (%d) in %s\n",
            (int)prim->type, path);
    cgltf_free(data);
    lua_pushnil(L);
    return 1;
