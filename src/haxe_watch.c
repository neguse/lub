...
static SDL_EnumerationResult SDLCALL enum_cb(void *userdata,
    ...
    if (!w || !dir || !fname) return SDL_ENUM_CONTINUE;

    char full[768];
    // `dir` の末尾が `/` を持っているか SDL は保証しない実装があるので、
    // 必ず明示的に separator を入れる。dir が既に `/` で終わっていた場合は
    // `<dir>//<fname>` になっても open には影響しない (POSIX 上は collapse される)。
...
bool haxe_watch_init(HaxeWatch *w, const char *hxml_path, const HxmlMeta *meta) {
    if (!w) return false;
    SDL_zerop(w);
    if (!hxml_path || !meta) return false;
...
bool haxe_watch_tick(HaxeWatch *w, bool *meta_dirty) {
    if (meta_dirty) *meta_dirty = false;
    ...
    for (int i = 0; i < w->count; ++i) {
        ...
        if (t != w->entries[i].mtime_ns) {
            ...
            if (i == 0 && meta_dirty) *meta_dirty = true;
