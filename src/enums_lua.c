...
static_assert((int)SGL_BLEND_NONE == (int)SGL_CULL_NONE,
              "NONE values must match for Lua-side sharing");
...
void enums_register(lua_State *L) {
  ...
  // respective enums, so a single Lua "NONE" works for either. The pipeline
  // builder disambiguates by which option key it appears under (blend vs cull).
