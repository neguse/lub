...
static void lub_sokol_logger(const char *tag, uint32_t level, uint32_t item_id,
...
static bool create_vk_instance(VkInstance *out_inst) {
  ...
  for (uint32_t i = 0; i < extra_count; ++i)
    all[sdl_ext_count + i] = extra_exts[i];
...
static bool pick_physical_device(VkInstance inst, VkPhysicalDevice *out_phys,
                                 uint32_t *out_qf) {
  uint32_t n = 0;
  vkEnumeratePhysicalDevices(inst, &n, NULL);
  if (n == 0) {
    SDL_Log("no Vulkan physical device");
    return false;
  }
  VkPhysicalDevice *phys =
      (VkPhysicalDevice *)malloc(sizeof(VkPhysicalDevice) * n);
  vkEnumeratePhysicalDevices(inst, &n, phys);

  for (uint32_t i = 0; i < n; ++i) {
    uint32_t qfn = 0;
    vkGetPhysicalDeviceQueueFamilyProperties(phys[i], &qfn, NULL);
    VkQueueFamilyProperties *qfp = (VkQueueFamilyProperties *)malloc(
        sizeof(VkQueueFamilyProperties) * qfn);
...
static bool create_surface(App *app) {
  if (!SDL_Vulkan_CreateSurface(app->window, app->vk_instance, NULL,
                                &app->vk_surface)) {
...
static void destroy_swapchain_resources(App *app) {
  ...
  if (app->vk_present_sems) {
    for (uint32_t i = 0; i < app->vk_swapchain_image_count; ++i) {
      if (app->vk_present_sems[i])
        vkDestroySemaphore(app->vk_device, app->vk_present_sems[i], NULL);
    }
  ...
  if (app->vk_acquire_sems) {
    for (uint32_t i = 0; i < app->vk_swapchain_image_count; ++i) {
      if (app->vk_acquire_sems[i])
        vkDestroySemaphore(app->vk_device, app->vk_acquire_sems[i], NULL);
    }
  ...
  if (app->vk_depth_mem) {
    vkFreeMemory(app->vk_device, app->vk_depth_mem, NULL);
    app->vk_depth_mem = VK_NULL_HANDLE;
  }
  ...
  if (app->vk_swapchain) {
    vkDestroySwapchainKHR(app->vk_device, app->vk_swapchain, NULL);
    app->vk_swapchain = VK_NULL_HANDLE;
  }
...
static bool create_swapchain_resources(App *app) {
  ...
  vkGetPhysicalDeviceSurfaceFormatsKHR(app->vk_phys, app->vk_surface,
                                       &fmt_count, NULL);
  if (fmt_count == 0) {
    SDL_Log("no surface formats");
    return false;
  }
  VkSurfaceFormatKHR *fmts =
      (VkSurfaceFormatKHR *)malloc(sizeof(VkSurfaceFormatKHR) * fmt_count);
  vkGetPhysicalDeviceSurfaceFormatsKHR(app->vk_phys, app->vk_surface,
                                       &fmt_count, fmts);
  ...
  vkGetPhysicalDeviceSurfaceCapabilitiesKHR(app->vk_phys, app->vk_surface,
                                            &caps);
  ...
  if (vkCreateSwapchainKHR(app->vk_device, &sci, NULL, &app->vk_swapchain) !=
      VK_SUCCESS) {
  ...
  vkGetSwapchainImagesKHR(app->vk_device, app->vk_swapchain,
                          &app->vk_swapchain_image_count, NULL);
  app->vk_swapchain_images =
      (VkImage *)malloc(sizeof(VkImage) * app->vk_swapchain_image_count);
  vkGetSwapchainImagesKHR(app->vk_device, app->vk_swapchain,
  ...
  app->vk_swapchain_views =
      (VkImageView *)calloc(app->vk_swapchain_image_count, sizeof(VkImageView));
  for (uint32_t i = 0; i < app->vk_swapchain_image_count; ++i) {
    ...
    if (vkCreateImageView(app->vk_device, &ivci, NULL,
                          &app->vk_swapchain_views[i]) != VK_SUCCESS) {
  ...
  if (vkCreateImage(app->vk_device, &dci, NULL, &app->vk_depth_image) !=
      VK_SUCCESS) {
  ...
  for (uint32_t i = 0; i < pmp.memoryTypeCount; ++i) {
    if ((mr.memoryTypeBits & (1u << i)) &&
        (pmp.memoryTypes[i].propertyFlags &
  ...
  if (vkAllocateMemory(app->vk_device, &mai, NULL, &app->vk_depth_mem) !=
      VK_SUCCESS) {
  ...
  if (vkCreateImageView(app->vk_device, &dvci, NULL, &app->vk_depth_view) !=
      VK_SUCCESS) {
  ...
  app->vk_acquire_sems =
      (VkSemaphore *)calloc(app->vk_swapchain_image_count, sizeof(VkSemaphore));
  app->vk_present_sems =
      (VkSemaphore *)calloc(app->vk_swapchain_image_count, sizeof(VkSemaphore));
  for (uint32_t i = 0; i < app->vk_swapchain_image_count; ++i) {
    if (vkCreateSemaphore(app->vk_device, &sci2, NULL,
                          &app->vk_acquire_sems[i]) != VK_SUCCESS) {
      SDL_Log("vkCreateSemaphore (acquire #%u) failed", i);
      return false;
    }
    if (vkCreateSemaphore(app->vk_device, &sci2, NULL,
                          &app->vk_present_sems[i]) != VK_SUCCESS) {
...
static bool recreate_swapchain_resources(App *app) {
  ...
  if (!create_swapchain_resources(app)) {
    destroy_swapchain_resources(
        app); // ensure clean slate after partial failure
    return false;
  }
  return true;
}
...
static bool create_vk_device(VkPhysicalDevice phys, uint32_t qf,
  ...
  VkDeviceCreateInfo ci = {
      ...
      .enabledExtensionCount =
          (uint32_t)(sizeof(dev_exts) / sizeof(dev_exts[0])),
...
static bool sk_init(App *app) {
  ...
  if (!create_vk_instance(&app->vk_instance))
    return false;
  if (!pick_physical_device(app->vk_instance, &app->vk_phys,
                            &app->vk_queue_family))
    return false;
  if (!create_vk_device(app->vk_phys, app->vk_queue_family, &app->vk_device,
                        &app->vk_queue))
    return false;
...
static void sk_begin_frame(App *app, int *out_w, int *out_h) {
  ...
  if (!app->vk_swapchain)
    return; // minimize / failed recreate; main loop skips frame

  uint32_t slot = (uint32_t)(app->frame_index % app->vk_swapchain_image_count);
  VkResult r = vkAcquireNextImageKHR(app->vk_device, app->vk_swapchain,
                                     UINT64_MAX, app->vk_acquire_sems[slot],
                                     VK_NULL_HANDLE, &app->vk_current_image);
  ...
  app->vk_last_presented_image =
      app->vk_swapchain_images[app->vk_current_image];
}
...
static BackendBuffer sk_make_buffer(SglBufferType type, const void *data,
                                    size_t bytes) {
...
static BackendImage sk_make_image(const ImageDesc *d) {
  ...
  case SGL_PF_RGBA16F:
    ...
    break;
  case SGL_PF_RGBA32F:
    pf = SG_PIXELFORMAT_RGBA32F;
    break;
  case SGL_PF_DEPTH16:
  case SGL_PF_DEPTH32F:
    pf = SG_PIXELFORMAT_DEPTH;
    break;
  case SGL_PF_DEPTH24_STENCIL8:
    pf = SG_PIXELFORMAT_DEPTH_STENCIL;
    break;
  case SGL_PF_RGBA8:
  default:
    pf = SG_PIXELFORMAT_RGBA8;
    break;
  }
  ...
  sg_filter sf =
      (d->filter == SGL_FILTER_NEAREST) ? SG_FILTER_NEAREST : SG_FILTER_LINEAR;
  sg_wrap sw =
      (d->wrap == SGL_WRAP_CLAMP) ? SG_WRAP_CLAMP_TO_EDGE : SG_WRAP_REPEAT;
...
static BackendShader sk_make_shader(const ShaderDesc *d) {
  ...
  if (is_compute) {
    ...
    // binding=slot.
    for (int i = 0; i < ss->refl.storage_buf_count && i < SG_MAX_VIEW_BINDSLOTS;
         ++i) {
...
static BackendPipeline sk_make_pipeline(const PipelineDesc *d) {
  ...
  if (d->has_depth) {
    ...
    desc.depth.compare =
        d->depth_test ? SG_COMPAREFUNC_LESS_EQUAL : SG_COMPAREFUNC_ALWAYS;
  ...
  desc.cull_mode = (d->cull == SGL_CULL_BACK)    ? SG_CULLMODE_BACK
  ...
  desc.primitive_type =
      ...
      : (d->primitive == SGL_PRIM_TRIANGLE_STRIP)
  ...
  if (d->refl) {
    for (int i = 0; i < d->refl->attr_count; ++i) {
      ...
      case 1:
        fmt = SG_VERTEXFORMAT_FLOAT;
        break;
      case 2:
        fmt = SG_VERTEXFORMAT_FLOAT2;
        break;
      case 3:
        fmt = SG_VERTEXFORMAT_FLOAT3;
        break;
      case 4:
        fmt = SG_VERTEXFORMAT_FLOAT4;
        break;
      default:
        fmt = SG_VERTEXFORMAT_FLOAT3;
      }
...
static void sk_begin_pass(App *app, const PassBeginDesc *d) {
  if (sk_try_begin_offscreen_pass(d))
    return;
  uint32_t slot = (uint32_t)(app->frame_index % app->vk_swapchain_image_count);
  sg_pass pass = {
      .action.colors[0] =
          {
              ...
              .clear_value = {d->clear[0][0], d->clear[0][1], d->clear[0][2],
                              d->clear[0][3]},
          },
      .swapchain =
          {
              ...
              .vulkan =
                  {
                      ...
                      .render_view =
                          (const void *)
                              app->vk_swapchain_views[app->vk_current_image],
                      .depth_stencil_image = (const void *)app->vk_depth_image,
                      .depth_stencil_view = (const void *)app->vk_depth_view,
                      .render_finished_semaphore =
                          (const void *)app->vk_present_sems[slot],
                      .present_complete_semaphore =
                          (const void *)app->vk_acquire_sems[slot],
                  },
          },
  };
  sg_begin_pass(&pass);
...
static void sk_dispatch(App *app, const ComputeDispatchDesc *d) {
  ...
  for (int i = 0; i < d->n_storage_bufs; ++i) {
    ...
    if (!buf || !buf->storage_view.id || !d->storage_bufs[i].name)
      continue;
...
static uint32_t pick_host_visible_memory_type(VkPhysicalDevice phys,
                                              uint32_t type_bits,
                                              bool *out_coherent) {
  VkPhysicalDeviceMemoryProperties pmp;
...
static void sk_begin_frame(App *app, int *out_w, int *out_h) {
  ...
  if (needs_resize) {
    ...
    lub_wgpu_configure_surface(app, (uint32_t)cw, (uint32_t)ch);
    if (!lub_wgpu_recreate_depth(app, (uint32_t)cw, (uint32_t)ch)) {
      SDL_Log("sk_begin_frame: depth recreate failed during resize; skipping "
              "frame");
      if (out_w)
        *out_w = cw;
      if (out_h)
  ...
  if (!ok) {
    ...
    if (surf_tex.status == WGPUSurfaceGetCurrentTextureStatus_Outdated ||
      ...
      lub_wgpu_configure_surface(app, (uint32_t)cw, (uint32_t)ch);
      if (!lub_wgpu_recreate_depth(app, (uint32_t)cw, (uint32_t)ch)) {
        SDL_Log("sk_begin_frame: depth recreate failed during retry; skipping "
                "frame");
        if (out_w)
          *out_w = cw;
        if (out_h)
  ...
  if (ok && surf_tex.texture) {
    ...
    if ((int)sw != cw || (int)sh != ch) {
      if (!lub_wgpu_recreate_depth(app, sw, sh)) {
        SDL_Log("sk_begin_frame: depth recreate failed for swapchain %ux%u", sw,
                sh);
      }
      cw = (int)sw;
      ch = (int)sh;
