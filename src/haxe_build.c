...
static bool read_file_to_string(const char *path, char **out_buf,
                                size_t *out_len) {
...
static bool concat_and_atomic_write(const char *raw_path, const char *out_path,
  ...
  bool write_ok = true;
  if (fwrite(HAXE_PRELUDE, 1, prelude_len, f) != prelude_len)
    write_ok = false;
  if (write_ok && raw_len > 0 && fwrite(raw, 1, raw_len, f) != raw_len)
    write_ok = false;
...
static void capture_process_output(SDL_Process *p, HaxeBuildResult *r) {
  ...
  for (;;) {
    ...
    if (n == 0)
      break;
    if (n > cap)
      n = cap; // 安全側 (sizeof(buf) < cap だが念のため)
    if (logged + n <= cap) {
...
HaxeBuildResult haxe_build_run(const HaxeServer *server, const char *hxml_path,
  ...
  if (!SDL_CreateDirectory(lub_dir)) {
    ...
    SDL_Log("haxe_build_run: SDL_CreateDirectory(%s) failed: %s", lub_dir,
            SDL_GetError());
  }
  ...
  if (props == 0) {
    SDL_snprintf(r.log, sizeof(r.log), "SDL_CreateProperties failed: %s",
                 SDL_GetError());
    return r;
  }
...
bool haxe_pipeline_tick(HaxePipeline *p) {
  ...
  if (!haxe_watch_tick(&p->watch, &meta_dirty))
    return false;
