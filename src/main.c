...
SDL_AppResult SDL_AppInit(void **appstate, int argc, char *argv[]) {
  ...
  if (!has_extension(entry_path, ".hxml")) {
    ...
    // 各サンプルは samples/<name>/ に自己完結し、Haxe は
    // samples/<name>/.lub/<name>.lua へ transpile される。bare-name entry
    // (例: WASM playground が渡す "12_sfb") もそこへ解決する。
    char sample_dir[512];
    ...
    SDL_snprintf(g_app.entry_path, sizeof(g_app.entry_path), "%s/.lub/%s.lua",
                 sample_dir, modbuf);
    g_app.entry_mtime_cache = 0;
    // package.path を <sample_dir>/.lub/?.lua に拡張 (.hxml branch
    // と同じ要領)。 tests/lua/*.lua の生スクリプトは boot.lua の package.path
    // で解決されるので ここで見つからなくても無害。
  ...
  if (!app_backend_init(&g_app))
    return SDL_APP_FAILURE; // initialize GPU backend after Lua onInit
