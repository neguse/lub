...
宣言も描画もされないフレームが `ResourceSweepAfterFrames`(既定 300)
続いたリソースは自動で破棄される。「作って解放する」ではなく「使っている
間だけ残る」モデル(詳細は「描画モデル」章)。`OnFrame` が error で抜けた
フレームの終わりには破棄しないので、編集中に error が続いている間に
リソースは消えない。
