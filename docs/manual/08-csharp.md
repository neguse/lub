...
細かい違いが出ることがある。`Renderer3d.Draw` は渡した行列をその場で写すので、
1 つの行列を `SetTrs` などで書き換えながら何度も `Draw` に渡してよい。
同じメッシュをたくさん描くときは、`InstanceBatch3d` に位置・回転・大きさ・色の
数値を積み、`Renderer3d.DrawInstances` でまとめて描くと、行列も作らず
draw も pass ごとに 1 回で済む。演算子と書き換える版の時間の差や、ほかに確保が
起きる書き方(クラスの `new`、式の途中の三項演算子、変更できる struct など)は
「コストの目安とホットパス」章にある。
