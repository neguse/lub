...
### 温め方: ゲームが宣言する(温め用起動オプション)

cache に何を入れるべきかの完全なリストは、動的コンテンツがある以上
ゲームにしか作れない。そして lub ではマニフェストはデータファイルでなく
コードなので、「リストの提出」は温め用の起動オプション(仮に --warm)で
表現する: runtime が「生成をすべて cache に記録するモード」で起動し、
ゲーム側の warm 経路(専用フック)が、使う予定のシェーダやアセット生成を
一通り踏む。ゲームが自分の閉包をコードで宣言する形。

- export = --warm 起動で cache を温める + ファイル閉包と一緒に凍結。
  手動プレイの結果に依存しないので CI で再現できる。
- 完全性の責任はゲーム側にある。ツールの仕事は保証ではなく可視化:
  dev セッション中に起きた生成のうち warm が踏んでいないものを差分として
  報告できる(構想)。生成器なし player での cache miss は「何が
  足りないか」を名指しするエラーにする。
- 実行ベースの完全性検証は担保にならないことを明記しておく。golden や
  headless 検証は数フレームしか回さないので、whole game のコードパス
  到達は証明できない。完全性は warm 経路の充実(ゲームの責務)でしか
  上がらない。
- 「Linux 上で DXIL を温められるか」という問題は残る。DXIL は DXC 依存で、
  DXC の Linux ビルドは存在するはずだが lub の現経路は dxcompiler.dll
  前提なので要検証。ただし native player が生成器を同梱している限り、
  export 機が DXIL を温められなくても win パッケージは正しく動く
  (初回起動が少し遅いだけ)。cook 方式だとここが「Linux からは win 向けを
  出せない」という機能欠損になる。
