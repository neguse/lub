...
開発が `lub game.hxml` で完結するのと同じ思想で、パッケージングも
CMake 等を要求しない。export は lub 自身の 1 コマンドで行う。
win パッケージは事前ビルド済み player を並べるだけなのでクロス
コンパイル不要(Linux 機からでも作れる)。web バンドルも同様。
