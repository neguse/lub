...
- project として `lub` という名前と目的が明確になっている。
...
- rename 後の build、最小実行、capture が通っている。
...
- input と timestep が game 固有 code から独立して検証できる。
- Haxe -> Lua 経由で同等の最小画面が動き、runtime API を Haxe 都合に寄せていない。
- deterministic な描画結果を capture / golden で確認できる。
...
- depth / render target の挙動が standalone sample または golden で確認できる。
...
- 既存 framework 互換を runtime API にしない。
- game-specific loader / adapter は runtime API にしない。
- core API は runtime が責任を持たないと壊れる primitive に限る。
- Haxe layer の都合で runtime API を歪めない。
- deterministic output が実用的な描画挙動は capture / golden で固定する。
